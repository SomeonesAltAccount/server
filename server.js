...
// World size is expressed in grid cells so it stays easy to resize -
// must match the client's GRID_SIZE (client.js) for the grid to line up.
const GRID_SIZE = 75;
...
wss.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        ...
        if (data.type === 'join') {
            ...
            const ALLOWED_SPECIES = ['mouse', 'shrimp', 'chipmunk'];
            const species = ALLOWED_SPECIES.includes(data.species) ? data.species : 'mouse';
            players[playerId] = {
